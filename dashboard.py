...
def process(raw):
    ...
    for folio in raw.get("folios", []):
        for scheme in folio.get("schemes", []):
            ...
            # SAFETY FIX: Ensure no NoneType crashes
            cost  = float(val.get("cost") or 0.0)
            value = float(val.get("value") or 0.0)
            units = float(scheme.get("close") or 0.0)
            ...
            for tx in txs:
                # SMART MATH TRANSACTION LOGIC (Safety patched for NoneType)
                raw_amt = float(tx.get("amount") or 0.0)
            ...
            # SAFETY FIX: Ensure c_nav doesn't crash on NoneType
            c_nav = float(val.get("nav") or 0.0)
            ...
            sip_txs = [
                t for t in txs
                # SAFETY FIX: Handle NoneType units in SIP check
                if float(t.get("units") or 0.0) > 0 and 
            ...
            if sip_txs:
                ...
                # SAFETY FIX: Handle NoneType in amount
                amt_sip    = float(latest.get("amount") or 0.0)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not tx: st.warning("No transaction data found.")
        ...
        st.dataframe(pd.DataFrame([{"Date": to_date(e.get("date")).strftime("%d %b %Y") if e.get("date") else "—", "Description": e.get("description", "—"), "Amount": fmt_inr(float(e.get("amount") or 0.0)) if e.get("amount") else "—", "NAV": f"₹{float(e.get('nav') or 0.0):,.4f}" if e.get("nav") else "—", "Units": f"{float(e.get('units') or 0.0):,.3f}" if e.get("units") else "—", "Type": e.get("type", "—")} for e in tx.get(sel, [])]), use_container_width=True, hide_index=True)
        st.markdown('</div>', unsafe_allow_html=True)
